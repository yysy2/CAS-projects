...
lamb = np.array([80,90,100,110,120,130])
...
def doWork((s_nr_it, s_method, s_lamb)):
   ...
   fmin = op.minimize(fun=cf.nnCostFunction, x0=initial_nn_params, args=(input_layer_size, hidden_layer_size, num_labels, X_train, y_train, lamb[s_lamb]), method=list_methods[s_method],jac=True, options={'maxiter': s_nr_it, 'disp': True})
   nn_params = fmin.x
