...
lamb = 100.0
fmin = op.minimize(fun=cf.nnCostFunction, x0=initial_nn_params, args=(input_layer_size, hidden_layer_size, num_labels, X, y, lamb), method='L-BFGS-B', jac=True, options={'maxiter': num_iterations, 'disp': True})
# print fmin
nn_params = fmin.x
...
pred = predict.nnpredict(Theta1, Theta2, X)
...
np.savetxt(str(int(lamb))+'_'+str(num_iterations)+'_'+"submission_train.csv", mysubmission_train.T, delimiter=",", fmt='%i')
...
p_test = predict.nnpredict(Theta1, Theta2, X_test);
print p_test
...
#     writer.writeheader()
np.set_printoptions(suppress=True)
np.savetxt(str(int(lamb))+'_'+str(num_iterations)+'_'+"submission_test.csv", mysubmission.T, delimiter=",", fmt='%i')
